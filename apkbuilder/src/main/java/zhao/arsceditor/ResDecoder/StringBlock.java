...
public class StringBlock {
    ...
    public ByteArrayOutputStream writeString(List<String> stringlist) throws IOException {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if ((flags & UTF8_FLAG) == 0) {
                ...
                len += var.getBytes(StandardCharsets.UTF_16LE).length;
                // 字符串在文件中储存的长度偏移2
                len += 2;
            } else { // 如果是UTF-8型
                ...
                len += 1;
                // 获取字符串在内存中的字节
                byte[] bytes = var.getBytes(StandardCharsets.UTF_8);
                // 获取该字符串占用字节的大小
                length = bytes.length;
                if (length > 0x0000007f) {
                    // 字符串在文件中储存的长度偏移1
    ...
    private void writeString(String str, LEDataOutputStream lmString) throws IOException {
        ...
        if ((flags & UTF8_FLAG) == 0) {
            ...
            lmString.writeFully(str.getBytes(StandardCharsets.UTF_16LE));
